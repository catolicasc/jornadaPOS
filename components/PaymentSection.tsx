...
export const PaymentSection = () => {
    ...
    const selected = useSelector((state: RootState) => state.payment.selectedMethod)
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.scroll}>
                {methods.map(({ label, icon }) => (
                    <TouchableOpacity
                        ...
                        onPress={() => dispatch(setPaymentMethod(label))}
                    >
                        <Ionicons name={icon} size={20} color={selected === label ? '#fff' : Colors.primary} />
                        <Text style={[styles.text, selected === label && styles.selectedText]}>
                            {label.toUpperCase()}
                        </Text>
                    </TouchableOpacity>
                ))}
